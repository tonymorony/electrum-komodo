...
;Variables
  ...
  !define PRODUCT_NAME "Komodo Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/komodoplatform/electrum"
  !define PRODUCT_PUBLISHER "KomodoPlatform"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-komodo-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-komodo\icons\electrum-komodo.ico"
...
Section
  ...
  File "..\..\icons\electrum-komodo.ico"
  ...
  ; CreateShortCut "file.lnk" "file.exe" "commandline params" "file.ico" "0" "SW_SHOWMAXIMIZED"
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-komodo.ico" "0"

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe" 0


  ;Links komodoplatform: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-komodo.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-komodo-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-komodo.ico"
